...
class ScaffoldStrategy(Strategy):
    ...
    def initialize_state(self, scene_scale: float = 1.0, n_offsets: int = 5, voxel_size: float = 0.001, anchor_feature_dim: int = 32 ) -> Dict[str, Any]:
        ...
        state = {"offset_grad2d": None, "scene_scale": scene_scale, "offset_demon": None, "opacity_accum": None, "anchor_demon": None, "n_offsets": None, "n_offsets": n_offsets, "voxel_size": voxel_size, "anchor_feature_dim": anchor_feature_dim}
    ...
    def check_sanity(
        ...
        for key in ["anchors", "scales", "quats", "opacities","offsets","anchor_features"]:
    ...
    def step_post_backward(
        ...
        if (
            ...
            if self.verbose:
                print(
                    f"Step {step}: {n_anchors} anchors split. "
    ...
    def _update_state(
        ...
        if state["offset_grad2d"] is None:
            state["offset_grad2d"] = torch.zeros((n_gaussian*state["n_offsets"],1), device=grads.device)
        if state["offset_demon"] is None:
            state["offset_demon"] = torch.zeros((n_gaussian*state["n_offsets"],1), device=grads.device)
        if state["opacity_accum"] is None:
            state["opacity_accum"] = torch.zeros((n_gaussian,1), device=grads.device)
        if state["anchor_demon"] is None:
            state["anchor_demon"] = torch.zeros((n_gaussian,1), device=grads.device)
        ...
        state["opacity_accum"].index_add_(
            0, anchor_ids, temp_opacity.sum(dim=1, keepdim=True)
        )

        state["anchor_demon"].index_add_(
            0, anchor_ids, torch.ones_like(anchor_ids.unsqueeze(-1), dtype=torch.float32)
        )
        anchor_visible_mask = anchor_visible_mask.unsqueeze(dim=1).repeat([1, state["n_offsets"]]).view(-1)
        combined_mask = torch.zeros_like(state["offset_grad2d"].squeeze(-1), dtype=torch.bool)
        ...
        state["offset_demon"].index_add_(
            0, offset_ids, torch.ones_like(offset_ids, dtype=torch.float32)
        )
        state["offset_grad2d"].index_add_(
            0, offset_ids, grads.squeeze(0)[info["update_filter"].squeeze(0),:2].norm(dim=-1)
        )
    ...
    def _grow_gs(
        ...
        grads = state["offset_grad2d"] / state["offset_demon"]
        device = grads.device
        grads[grads.isnan()] = 0.0
        grads_norm = torch.norm(grads, dim=-1)
        offset_mask = (state["offset_demon"] > self.check_interval*self.success_threshold*0.5).squeeze(dim=1)
        ...
        for i in range(self.update_depth):
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                new_feat = params["anchor_features"].unsqueeze(dim=1).repeat([1, state["n_offsets"], 1]).view([-1, state["anchor_feature_dim"]])[candidate_mask]

                new_feat = scatter_max(new_feat, inverse_indices.unsqueeze(1).expand(-1, new_feat.size(1)), dim=0)[0][remove_duplicates]

                new_offsets = torch.zeros_like(candidate_anchor).unsqueeze(dim=1).repeat([1,state["n_offsets"],1]).float().cuda()
                ...
                temp_anchor_demon = torch.cat([state["anchor_demon"], torch.zeros([new_opacities.shape[0], 1], device=device).float()], dim=0)
                state["anchor_demon"]= temp_anchor_demon

                temp_opacity_accum = torch.cat([state["opacity_accum"], torch.zeros([new_opacities.shape[0], 1], device=device).float()], dim=0)
        ...
        padding_offset_demon = torch.zeros([params["anchors"].shape[0]*state["n_offsets"] - state["offset_demon"].shape[0], 1],
                                           dtype=torch.int32, 
                                           device=device)
        state["offset_demon"] = torch.cat([state["offset_demon"], padding_offset_demon], dim=0)

        state["offset_grad2d"][offset_mask] = 0
        padding_offset_gradient_accum = torch.zeros([params["anchors"].shape[0]*state["n_offsets"] - state["offset_grad2d"].shape[0], 1],
                                           dtype=torch.int32, 
                                           device=device)
        state["offset_grad2d"] = torch.cat([state["offset_grad2d"], padding_offset_gradient_accum], dim=0)
        
        # # prune anchors
        prune_mask = (state["opacity_accum"] < self.prune_opa*state["anchor_demon"]).squeeze(dim=1)
        anchors_mask = (state["anchor_demon"] > self.check_interval*self.success_threshold).squeeze(dim=1) # [N, 1]
        prune_mask = torch.logical_and(prune_mask, anchors_mask) # [N] 
        
        # update offset_demon
        offset_demon = state["offset_demon"].view([-1, state["n_offsets"]])[~prune_mask]
        offset_demon = offset_demon.view([-1, 1])
        state["offset_demon"] = offset_demon

        offset_gradient_accum = state["offset_grad2d"].view([-1, state["n_offsets"]])[~prune_mask]
        offset_gradient_accum = offset_gradient_accum.view([-1, 1])
        state["offset_grad2d"] = offset_gradient_accum
        
        # update opacity accum 
        if anchors_mask.sum()>0:
            state["opacity_accum"][anchors_mask] = torch.zeros([anchors_mask.sum(), 1], device=device).float()
            state["anchor_demon"][anchors_mask] = torch.zeros([anchors_mask.sum(), 1], device=device).float()
    ...
    def grow_anchors(
        ...
        def param_fn(name: str, p: torch.Tensor) -> torch.Tensor:
            if name in ["anchors","scales","quats","opacities","offsets","anchor_features"]:
