...
class Config:
    ...
    strategy: ScaffoldStrategy = field(
        default_factory=ScaffoldStrategy
    )
...
def create_splats_with_optimizers(
    ...
    mlp_opacity = torch.nn.Sequential(
        torch.nn.Linear(anchor_feature_dim+3+opacity_dist_dim, anchor_feature_dim),
        torch.nn.ReLU(True),
    ...
    mlp_cov = torch.nn.Sequential(
        torch.nn.Linear(anchor_feature_dim+3+cov_dist_dim, anchor_feature_dim),
        torch.nn.ReLU(True),
    ...
    appearance_dim = feature_dim if feature_dim is not None else 0
    mlp_color = torch.nn.Sequential(
        torch.nn.Linear(anchor_feature_dim+3+color_dist_dim+appearance_dim, anchor_feature_dim),
        torch.nn.ReLU(True),
    ...
    # TODO lr rate schedule is not aligned with official scaffold-GS
    optimizers = {
...
def save_ply(splats: torch.nn.ParameterDict, decoders: torch.nn.ModuleDict, path: str):
    from plyfile import PlyData, PlyElement
    anchors = splats["anchors"].detach().cpu().numpy()
    normals = np.zeros_like(anchors)
    anchor_feat = splats["anchor_features"].detach().cpu().numpy()
    offset = splats["offsets"].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def construct_list_of_attributes(splats):
        ...
        for i in range(splats["offsets"].shape[1]*splats["offsets"].shape[2]):
            l.append('f_offset_{}'.format(i))
        for i in range(splats["anchor_features"].shape[1]):
            l.append('f_anchor_feat{}'.format(i))
        l.append('opacity')
    ...
    elements = np.empty(anchors.shape[0], dtype=dtype_full)
    attributes = np.concatenate((anchors, normals, offset, anchor_feat, opacities, scale, rotation), axis=1)
    elements[:] = list(map(tuple, attributes))
    el = PlyElement.describe(elements, 'vertex')
    PlyData([el]).write(path)

    torch.save(decoders.state_dict(), os.path.join(os.path.dirname(path), 'checkpoints.pth'))
...
class Runner:
    ...
    def __init__(
        ...
        # TODO
        if isinstance(self.cfg.strategy, ScaffoldStrategy):
    ...
    def get_neural_gaussians(
        ...
        info = {
            ...
            "anchor_visible_mask": visible_anchor_mask,
        }
        return info
    ...
    def train(self):
        ...
        with self.get_profiler(self.writer) as prof:
            ...
            for step in pbar:
                ...
                loss = l1loss * (1.0 - cfg.ssim_lambda) + ssimloss * cfg.ssim_lambda + info["scales"].prod(dim=1).mean() * cfg.scale_reg
                if cfg.depth_loss:
                ...
                # TODO make sure before optimization or after
                if isinstance(self.cfg.strategy, ScaffoldStrategy):
    ...
    def save_params_into_ply_file(
        ...
        save_ply(self.splats, self.decoders, ply_file)
        print(f"Saved parameters of splats into file: {ply_file}.")
        print(f"Saved nets weight into file: {os.path.join(os.path.dirname(ply_file),'checkpoints.pth')}.")
...
if __name__ == "__main__":
    ...
    configs = {
        "Scaffold": (
            "Scaffold",
            Config(
                strategy=ScaffoldStrategy(verbose=True),
                n_offsets=5,
                anchor_feature_dim=48,
            ),
        ),
        "default": (
            "Scaffold with compression.",
            Config(
            ),
        ),
    }
